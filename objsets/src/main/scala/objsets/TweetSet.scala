...
class Empty extends TweetSet:
  ...
  def mostRetweeted: Tweet = throw new ju.NoSuchElementException
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet:
  ...
  //     .union(right.filterAcc(p, acc.incl(elem)))
  //   else
  ...
  def sort(predecessor: TweetSet, t: Tweet) : TweetSet =
    if t.retweets < elem.retweets then
      sort(NonEmpty(elem, predecessor, right), elem)
    else
      predecessor
  ...
  def mostRetweeted: Tweet =
    def max(ts: TweetSet, t: Tweet): Tweet =
      if(elem.retweets > t.retweets) then
        max(ts, elem)
      else t
